...
public class EmailUtils {
	...
	public static void sendEmail(User user){
		//发送方
		String myAccount = "2739175034@qq.com";

		//授权码, 这个授权码,参照课上的获取方式,不要用我的.
		// 不要用我的.大哥们.
		// 不要用我的.大哥们.
		// 不要用我的.大哥们.
		// 不要用我的.大哥们.
		...
		// 不要用我的.大哥们.
		// locknxihzpaabicc
		String myPass = "agmzdxkvtxtwdgch";
		//发件人 邮箱的 SMTP 服务器地址
		// 不同和邮件类型地址不一样.qq是这个,126的跟这不是一个.推荐大家使用邮箱.
	...
	private static MimeMessage createMsg(Session session, String myAccount, User user) {
		...
		try {
			...
			String url = "http://"+ip+":8080/user?method=activate&e="+ Base64Utils.encode(user.getEmail())+"&c="+Base64Utils.encode(user.getCode());
